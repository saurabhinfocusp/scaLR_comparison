...
print('Running CellTypist SGD Regression model...')
start_time = time.time()
model = celltypist.train(a_data_train, labels = target, n_jobs = 1, use_SGD=True, feature_selection = False, check_expression=False)
model_running_time = time.time() - start_time
print(f'Model training time : {model_running_time/(60)} mins')

# %%
# Save the model.
model.write(f'{store_results_path}/sgd_model.pkl')
...
pd.DataFrame(classification_report(test_targets, test_preds, output_dict=True)).transpose().to_csv(f'{store_results_path}/results/sgd_{target}_top_3500_feat_stats.csv')
