...
# %%
# The `target` in `adata_2000.obs` will be used as cell Type labels for training.
print('Running CellTypist Logistic Regression model on top selected features training data - all features...')
start_time = time.time()
model = celltypist.train(a_data_train, labels = target, n_jobs = 1, feature_selection = False, check_expression=False)
model_running_time = time.time() - start_time
print(f'Model training time : {model_running_time/(60)} mins')

# %%
# Save the model.
model.write(f'{store_results_path}/logistic_model.pkl')
...
# %%
pd.DataFrame(classification_report(test_targets, test_preds, output_dict=True)).transpose()

# %%
if not os.path.exists(f'{store_results_path}/results'):
    os.makedirs(f'{store_results_path}/results', exist_ok=True)

pd.DataFrame(classification_report(test_targets, test_preds, output_dict=True)).transpose().to_csv(f'{store_results_path}/results/logistic_{target}_all_features_stats.csv')
