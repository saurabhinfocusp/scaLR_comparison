...
ten_folds_predictions = []

for i in range(10):
    print(f'\n\nTrial {i} running...')
    pred_label = scBalance(test, reference, ref_label, processing_unit='gpu')
    print(f'\nAccuracy - trial {i}: ', (accuracy_score(test_label, pred_label)))
    ten_folds_predictions.append(pred_label)
    print('\n\n')

ten_fold_test_acc = []
for pred_label in ten_folds_predictions:
    ten_fold_test_acc.append((accuracy_score(test_label, pred_label)))

print('\n\nMean accuracy across 10 trials : ', np.mean(ten_fold_test_acc))